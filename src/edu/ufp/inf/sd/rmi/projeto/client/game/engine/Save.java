...
public class Save {
	...
	private String SavePlayerData() {
		...
		for (int i = 0; i < Game.btl.totalplayers; i++) {
			edu.ufp.inf.sd.rmi.projeto.client.game.players.Base ply = Game.player.get(i);
	...
	private String SaveCityData() {
		...
		for (int i = 0; i < Game.builds.size(); i++) {
			edu.ufp.inf.sd.rmi.projeto.client.game.buildings.Base bld = Game.builds.get(i);
	...
	private String SaveUnitData() {
	    ...
	    for (int i = 0; i < Game.units.size(); i++) {
			edu.ufp.inf.sd.rmi.projeto.client.game.units.Base unit = Game.units.get(i);
