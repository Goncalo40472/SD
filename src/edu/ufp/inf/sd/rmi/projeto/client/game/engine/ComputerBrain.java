...
public class ComputerBrain {
	...
	public void ThinkDamnYou(edu.ufp.inf.sd.rmi.projeto.client.game.players.Base ply) throws RemoteException {
		...
		if (!DoneUnits) {
			edu.ufp.inf.sd.rmi.projeto.client.game.units.Base unit = Game.units.get(UnitCount);
			UnitCount++;
			...
			edu.ufp.inf.sd.rmi.projeto.client.game.buildings.Base bld = Game.builds.get(CityCount);
			CityCount++;
	...
	private boolean WannaHug(edu.ufp.inf.sd.rmi.projeto.client.game.units.Base unit) {
	...
	private boolean BuildingInRange(edu.ufp.inf.sd.rmi.projeto.client.game.units.Base unit) {
