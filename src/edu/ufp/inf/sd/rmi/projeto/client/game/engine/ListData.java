...
import edu.ufp.inf.sd.rmi.projeto.client.game.units.*;
import edu.ufp.inf.sd.rmi.projeto.client.game.buildings.*;
...
public class ListData {
	 ...
	 */
	public edu.ufp.inf.sd.rmi.projeto.client.game.players.Base CreateCommander(int which,int team, int money, boolean npc) {
		switch (which) {
			case 0:return new Andy(npc,team,money);
			case 1:return new Colin(npc,team,money);
			case 2:return new Max(npc,team,money);
			case 3:return new Sammi(npc,team,money);
		}
		return null;
	}
	/**
	 ...
	 */
	public edu.ufp.inf.sd.rmi.projeto.client.game.buildings.Base CreateCity(int owner, int xx, int yy, int type) {//15 = Neutral, 12, 13, 14 are unused. (12 max players)
		//TODO: Warning: I removed the check to make sure each building is legal.
		System.out.println("List level owner: " + owner);
		switch (type) {
			case 0:return new Capital(owner, xx, yy);
			case 1:return new Town(owner, xx, yy);
			case 2:return new Barracks(owner, xx, yy);
			case 3:return new Airport(owner, xx, yy);
			case 4:return new Shipyard(owner, xx, yy);
		}
		return null;
	}
	/**
	 ...
	 */
	public edu.ufp.inf.sd.rmi.projeto.client.game.units.Base CreateUnit(int type, int owner, int xx, int yy, boolean active) {
		switch (type) {
			case 0:return new Infantry(owner, xx, yy, active);
			case 1:return new Mechanic(owner, xx, yy, active);
			case 2:return new SmallTank(owner, xx, yy, active);
			case 3:return new Artillery(owner, xx, yy, active);
			case 4:return new Helecopter(owner, xx, yy, active);
		}
		return null;
	}
	...
	public void LoadCommander(String Name, boolean Defeated, int Team, int Money, int Kills, int Loses, int Power, int Using, boolean npc) {
		for (int i = 0; i < Game.displayC.size(); i++) {
			if (Game.displayC.get(i).name.equals(Name)) {
				...
				edu.ufp.inf.sd.rmi.projeto.client.game.players.Base ply = Game.player.get(Game.player.size()-1);
	...
	public void LoadCity(String Name, int Owner, int Health, int id) {
		...
		edu.ufp.inf.sd.rmi.projeto.client.game.buildings.Base bld = Game.builds.get(id);
		bld.owner = Owner;
		bld.health = Health;
	}
	...
	public void LoadUnit(String Name, int Owner, int Health, int Ammo, int Fuel, int X, int Y, boolean Acted) {
		for (int i = 0; i < Game.displayU.size(); i++) {
			if (Game.displayU.get(i).name.equals(Name)) {
				...
				edu.ufp.inf.sd.rmi.projeto.client.game.units.Base unit = Game.units.get(Game.units.size()-1);
				unit.health = Health;
				unit.Ammo = Ammo;
				unit.Fuel = Fuel;
			}
