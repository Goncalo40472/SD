...
public class ListData {
	 ...
	 */
	public edu.ufp.inf.sd.rabbitmqservices.projeto.game.players.Base CreateCommander(int which,int team, int money, boolean npc) {
		switch (which) {
			case 0:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.players.Andy(npc,team,money);
			case 1:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.players.Colin(npc,team,money);
			case 2:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.players.Max(npc,team,money);
			case 3:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.players.Sammi(npc,team,money);
		}
		return null;
	}
	/**
	 ...
	 */
	public edu.ufp.inf.sd.rabbitmqservices.projeto.game.buildings.Base CreateCity(int owner, int xx, int yy, int type) {//15 = Neutral, 12, 13, 14 are unused. (12 max players)
		//TODO: Warning: I removed the check to make sure each building is legal.
		System.out.println("List level owner: " + owner);
		switch (type) {
			case 0:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.buildings.Capital(owner, xx, yy);
			case 1:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.buildings.Town(owner, xx, yy);
			case 2:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.buildings.Barracks(owner, xx, yy);
			case 3:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.buildings.Airport(owner, xx, yy);
			case 4:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.buildings.Shipyard(owner, xx, yy);
		}
		return null;
	}
	/**
	 ...
	 */
	public edu.ufp.inf.sd.rabbitmqservices.projeto.game.units.Base CreateUnit(int type, int owner, int xx, int yy, boolean active) {
		switch (type) {
			case 0:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.units.Infantry(owner, xx, yy, active);
			case 1:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.units.Mechanic(owner, xx, yy, active);
			case 2:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.units.SmallTank(owner, xx, yy, active);
			case 3:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.units.Artillery(owner, xx, yy, active);
			case 4:return new edu.ufp.inf.sd.rabbitmqservices.projeto.game.units.Helecopter(owner, xx, yy, active);
		}
		return null;
	}
	...
	public void LoadCommander(String Name, boolean Defeated, int Team, int Money, int Kills, int Loses, int Power, int Using, boolean npc) {
		for (int i = 0; i < Game.displayC.size(); i++) {
			if (Game.displayC.get(i).name.equals(Name)) {
				...
				edu.ufp.inf.sd.rabbitmqservices.projeto.game.players.Base ply = Game.player.get(Game.player.size()-1);
	...
	public void LoadCity(String Name, int Owner, int Health, int id) {
		...
		edu.ufp.inf.sd.rabbitmqservices.projeto.game.buildings.Base bld = Game.builds.get(id);
		bld.owner = Owner;
		bld.health = Health;
	}
	...
	public void LoadUnit(String Name, int Owner, int Health, int Ammo, int Fuel, int X, int Y, boolean Acted) {
		for (int i = 0; i < Game.displayU.size(); i++) {
			if (Game.displayU.get(i).name.equals(Name)) {
				...
				edu.ufp.inf.sd.rabbitmqservices.projeto.game.units.Base unit = Game.units.get(Game.units.size()-1);
				unit.health = Health;
				unit.Ammo = Ammo;
				unit.Fuel = Fuel;
			}
