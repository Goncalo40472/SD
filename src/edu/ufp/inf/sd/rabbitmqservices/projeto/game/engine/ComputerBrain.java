...
import edu.ufp.inf.sd.rabbitmqservices.projeto.game.units.Base;
...
public class ComputerBrain {
	...
	public void ThinkDamnYou(edu.ufp.inf.sd.rabbitmqservices.projeto.game.players.Base ply) {
		...
		if (!DoneUnits) {
			Base unit = Game.units.get(UnitCount);
			UnitCount++;
			...
			edu.ufp.inf.sd.rabbitmqservices.projeto.game.buildings.Base bld = Game.builds.get(CityCount);
			CityCount++;
	...
	private void HandleBuilding(edu.ufp.inf.sd.rabbitmqservices.projeto.game.buildings.Base bld) {
		if (!bld.Menu.equals("")) {
			if (!bld.Locked) {
