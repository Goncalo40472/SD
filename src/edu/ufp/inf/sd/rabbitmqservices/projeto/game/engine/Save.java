 ...
 */
public class Save {
	final String path = "C:\\Users\\gonca\\IdeaProjects\\SD\\saves\\";
	...
	private String SavePlayerData() {
		...
		for (int i = 0; i < Game.btl.totalplayers; i++) {
			edu.ufp.inf.sd.rabbitmqservices.projeto.game.players.Base ply = Game.player.get(i);
	...
	private String SaveCityData() {
		...
		for (int i = 0; i < Game.builds.size(); i++) {
			edu.ufp.inf.sd.rabbitmqservices.projeto.game.buildings.Base bld = Game.builds.get(i);
	...
	private String SaveUnitData() {
	    ...
	    for (int i = 0; i < Game.units.size(); i++) {
			edu.ufp.inf.sd.rabbitmqservices.projeto.game.units.Base unit = Game.units.get(i);
	...
	public void LoadGame() {
		try {
			...
			Game.units = new ArrayList<edu.ufp.inf.sd.rabbitmqservices.projeto.game.units.Base>();
