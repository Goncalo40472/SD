package edu.ufp.inf.sd.rabbitmqservices.projeto.game.gui;
...
import edu.ufp.inf.sd.rabbitmqservices.projeto.game.engine.Pathfinding;
...
public class Ranges {
	public static void Draw(Graphics2D gg, int resize) {
		if (Game.player.get(Game.btl.currentplayer).unitselected) {
			...
			Base unit = Game.units.get(Game.player.get(Game.btl.currentplayer).selectedunit);
	...
	private static void Moving(Graphics2D gg, Base unit, int size, int resize) {
