...
import edu.ufp.inf.sd.rabbitmqservices.projeto.game.gui.*;
...
public class Gui extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		case PLAYING:
			...
			Terrain.Draw(gg, Game.ScreenBase);
			Ranges.Draw(gg, Game.ScreenBase);
			Cities.Draw(gg, Game.ScreenBase);
			Units.Draw(gg, Game.ScreenBase);
			Selector.Draw(frame, gg, Game.ScreenBase,
					Game.player.get(Game.btl.currentplayer).selectx,
					Game.player.get(Game.btl.currentplayer).selecty);
			SmallMenu.Draw(gg,frame);
		break;
		case EDITOR:
			gg.setColor(new Color(0,0,60));
			gg.fillRect(0, 0, width, height);
