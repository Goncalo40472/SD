package edu.ufp.inf.sd.rabbitmqservices.projeto.game.units;
...
public class Base {
	...
	public boolean PathCheck(int destx, int desty) {
		...
		for (edu.ufp.inf.sd.rabbitmqservices.projeto.game.units.Base unit : Game.units) {//Allows units in the same team to walk past each other.
	...
	public boolean attack(int destx, int desty, boolean returnfire) {
		...
		edu.ufp.inf.sd.rabbitmqservices.projeto.game.units.Base target = FindTarget(destx, desty, true, false);
		if (target!=null) {
	...
	private void capture(int destx,int desty) {
		...
		if (destx==x&&desty==y) {
			if (bld!=-1) {
				edu.ufp.inf.sd.rabbitmqservices.projeto.game.buildings.Base city = Game.builds.get(bld);
	...
	private edu.ufp.inf.sd.rabbitmqservices.projeto.game.units.Base FindTarget(int destx, int desty, boolean hostilefire, boolean friendlyfire) {
